	...
	for (int i = 0; i<len; ++i) {
		if (s[i] == '_') {
			...
			if (isdigit(s[i])) {
				...
				if (flag == 0) {
					int tempOperandNegative = -abs(multiCharDigitToNumber(1, operandStr)); // int absolute
					st.push(tempOperandNegative);
				}
				else {
					double tempOperandNegative = -abs(multiCharDigitToNumber(0.0, operandStr)); // double absolute
					st.push(tempOperandNegative);
				}
			}
		}
